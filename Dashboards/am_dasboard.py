...
## 1.0 LOAD DATA
################################################################################
#lpw_data = pd.read_csv('/content/drive/MyDrive/LPW/Chris LPW Focused Divisional structure and Metrics for 2021 draft April 2022.xlsx - dataset - Chris LPW Focused Divisional structure and Metrics for 2021 draft April 2022.xlsx - dataset.csv')
...
## Set title
st.title('LPW ACCOUNT MANAGER 2021 ANALYTICS DASHBOARD')

option = st.selectbox(
     'Select Account Manager',
     lpw_data['Account Manager'].unique())

st.write('Account Manager selected:', option)

lpw_data = lpw_data.loc[lpw_data['Account Manager'] == option]
...
kpi1.metric(label="New Coaches Count",value=f"{new_coaches_count:,}")

# define Seaborn color palette to use
#define Seaborn color palette to use
#colors = sns.color_palette('pastel')[0:len(coaches_by_am)]
  
# plotting data on chart
#fig1 = plt.figure(figsize=(3,3))
#plt.pie(coaches_by_am['new_coaches_count'],labels=coaches_by_am['Account Manager'], colors=colors, autopct='%.0f%%')
#plt.title('Account Managers Coaches Share 2021')
...
# define Seaborn color palette to use
#define Seaborn color palette to use
#colors = sns.color_palette('pastel')[0:len(stopped_df)]
  
# plotting data on chart
#fig2 = plt.figure(figsize=(3,3))
#plt.pie(stopped_df['count'],labels=stopped_df['count'], colors=colors, autopct='%.0f%%')
#plt.title('Churn Rate 2021')
#plt.show()
...
#ax = sns.barplot(x="Attrition Percent", y="Account Manager", data=am_attrition)
...
## BAR CHART
#####################################
#bar_chart1,piechart3 = st.columns(2)
...
#attriting.loc[attriting['time_on_platform'].isnull()]
#attriting.loc[attriting['time_on_platform'] < 0]
...
#KPI 2
#######################
# Average days for attriting coaches
kpi2.metric(label="Average Days on Platform for Cancelled Coaches",value=f"{np.ceil(mean_days):,}")
...
# Plot histogram
#sns.histplot(data = attriting2['time_on_platform'], kde = True)
#plt.title('Distribution of Days on Platform for Cancelled Coaches')
#plt.show()
...
#country_dist2 = country_dist
country_dist.sort_values('count',ascending=True,inplace=True)

# define Seaborn color palette to use
#define Seaborn color palette to use
#colors = sns.color_palette('pastel')[0:len(country_dist)]
  
# plotting data on chart
#plt.pie(country_dist['count'],labels=country_dist['Country'], colors=colors, autopct='%.0f%%')
#plt.title('Coaches Country Distribution 2021')
#plt.show()
...
#plt.show()
#heatmap1 = px.imshow(calls, text_auto=True)
...
#barchart2.plotly_chart(fig_calls_count)

#ax = sns.barplot(x="Percent", y="Stage", data=calls_count)
#ax.bar_label(ax.containers[0])
#plt.title('Onboarding Calls Transition Percentages')
#plt.show()
####################################
## Role Play (RP)
####################################
...
#barchart3.plotly_chart(fig_rp_count)
#ax = sns.barplot(x="Percent", y="Stage", data=rp_count)
#ax.bar_label(ax.containers[0])
#plt.title('Role Play Transition Percentages')
#plt.show()
######################################
## Lead Generation (LG)
######################################
...
heatmap3 = px.imshow(lg,aspect="auto",color_continuous_scale='Blues')#,text_auto=True
...
#barchart4.plotly_chart(fig_lg_count)

#ax = sns.barplot(x="Percent", y="Stage", data=lg_count)
#ax.bar_label(ax.containers[0])
#plt.title('Lead Generation Transition Percentages')
#plt.show()
...
#sns.heatmap(cohort_table4,annot=True,cmap="Blues")
